...
public class OrbitIdentification {
	...
	private static int readOrder = 0;

	public static final String DEFAULT_ORBIT_FILE = "/resources/Przulj.txt";

	/**
	 * Reads all orbits in from file and stores them by number for quick reference.
	 */
	public static void readGraphlets(String filename, int maxOrder) {
		if ((file==null && filename !=null)|| !(file== null || file.equals(filename))
				|| maxOrder > readOrder) {
			file = filename;
			readOrder = maxOrder;
	 ...
	 * @return The number of orbits in graphlets of no higher than the given order
	 */
	...
	private static Set<Set<Edge>> calculateSymmetry(OrbitRepresentative or) {
		...
		// List<SortedSet<Integer>> orbitsTemporary = new
		// ArrayList<SortedSet<Integer>>();
